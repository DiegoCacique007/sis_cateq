...
@section('title', 'Dashboard - Coordinador General')
@section('header_title', 'BIENVENIDO, COORDINADOR GENERAL')
...
@section('content')
    ...
    <div id="sec-inicio" class="crud-section" style="display: block;">
        <div class="card card-parroquia mb-4 shadow-sm border-0" style="background: linear-gradient(135deg, var(--blue-main) 0%, var(--blue-dark) 100%) !important;">
            <div class="card-body p-4 p-md-5 text-white">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h3 class="fw-bold mb-3 text-white">¡Bienvenido al Sistema Escolar!</h3>
                        <p class="mb-0 fs-6" style="color: rgba(255, 255, 255, 0.85);">
                            Desde este panel central puedes administrar de forma rápida y segura toda la configuración académica de la catequesis. Selecciona un módulo en el menú lateral o usa los accesos rápidos a continuación.
                        </p>
                    </div>
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-4">
                <div class="card card-parroquia border-0 shadow-sm h-100 p-2" style="cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'" onclick="document.querySelector('a[href$=\'#niveles\']').click()">
                    <div class="card-body text-center">
                        ...
                        <h6 class="fw-bold" style="color: var(--blue-dark);">Gestión de Niveles</h6>
                        <p class="text-muted small mb-0">Administra y crea nuevos niveles de catequesis.</p>
                    </div>
                </div>
            </div>

            <div class="col-md-4">
                <div class="card card-parroquia border-0 shadow-sm h-100 p-2" style="cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'" onclick="document.querySelector('a[href$=\'#periodos\']').click()">
                    <div class="card-body text-center">
                        ...
                        <h6 class="fw-bold" style="color: var(--blue-dark);">Periodos Escolares</h6>
                        <p class="text-muted small mb-0">Establece las fechas de inicio y fin de ciclos.</p>
                    </div>
                </div>
            </div>

            <div class="col-md-4">
                <div class="card card-parroquia border-0 shadow-sm h-100 p-2" style="cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'" onclick="document.querySelector('a[href$=\'#asigna_grupo\']').click()">
                    <div class="card-body text-center">
                        ...
                        <h6 class="fw-bold" style="color: var(--blue-dark);">Asignar Grupos</h6>
                        <p class="text-muted small mb-0">Vincula grupos, niveles, comunidades y catequistas.</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        const dbSchema = {
            niveles: [ { field: 'nivel', type: 'text', label: 'Nombre del Nivel' } ],
            periodos: [
                { field: 'fecha_inicio', type: 'date', label: 'Fecha de inicio' },
                { field: 'fecha_fin', type: 'date', label: 'Fecha de fin' },
                { field: 'estado', type: 'select', label: 'Estado', options: [{val: 1, text: 'Activo'}, {val: 0, text: 'Inactivo'}] }
            ],
            rubros: [
                { field: 'nombre', type: 'text', label: 'Nombre del Rubro' },
                { field: 'valor', type: 'number', step: '0.01', label: 'Porcentaje (%)' }
            ],
            unidades: [
                { field: 'nombre', type: 'text', label: 'Nombre de la unidad' },
                { field: 'numero', type: 'number', label: 'Número de Unidad' },
                { field: 'nivel_id', type: 'fk', label: 'Nivel', fkSource: 'niveles' }
            ],
            grupos: [ { field: 'nombre', type: 'text', label: 'Nombre del Grupo' } ],

        };

        const nombresBonitos = {
            niveles: 'Niveles', periodos: 'Periodos', rubros: 'Rubros',
            unidades: 'Unidades', grupos: 'Grupos', asigna_grupo: 'Asignación de grupos'
        };

        document.addEventListener("DOMContentLoaded", async function() {
            try {
                const res = await fetch('/coordinador-general/catalogos');
        ...
        async function cargarTabla(tabla) {
            ...
            try {
                const response = await fetch(`/coordinador-general/${tabla}`);
        ...
        function renderizarPaginacion(totalPages) {
            ...
            // Lógica para mostrar siempre unas cuantas páginas y evitar que se desborde
        ...
        async function guardarRegistro(event) {
            ...
            let url = `/coordinador-general/${activeTableGlobal}`;
            if (editandoId) {
                url = `/coordinador-general/${activeTableGlobal}/${editandoId}`;
            ...
            try {
                ...
                if (response.ok) {
                    const resFk = await fetch('/coordinador-general/catalogos');
                    fkData = await resFk.json();
                    crudModalInstance.hide();
                    cargarTabla(activeTableGlobal); // Recarga todo para actualizar la tabla y paginación
                } else {
        ...
        async function eliminarRegistro(id) {
            ...
            try {
                const response = await fetch(`/coordinador-general/${activeTableGlobal}/${id}`, {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value, 'Accept': 'application/json' }
                });
